...
def process(s1, s2, s3, midterm, final):
    ...
    print(f'Average With Lowest Dropped: {drop_ave:.2f}')

    better_ave = max(average, drop_ave)
    print(f"Using Better Average: {better_ave:.2f}")

    print('\n')
    
    weighted_ave = calculate_weighted(better_ave, midterm, final)
    print(f"Weighted Course Average: {weighted_ave:.2f}")
    grade = determine_grade(better_ave)
    print(f"Letter Grade: {grade}")
    points_needed = 90 - weighted_ave
    print('\n')

    improve = needs_improvement(better_ave, 90)
    print(f"Needs improvement for an 'A': {improve}")
